...
#define READ_USER_COMMANDS 0xC3
...
int requestData(int fileStream, int dataType){
...
int sendData(int fileStream, int dataType, char *data, int size){
    ...
    buffer[2] = dataType;
    memcpy(&buffer[3], matricula, 4);
    memcpy(&buffer[3 + 4], data, size);

    short crc = calcula_CRC(&buffer[0], 3 + 4 + size);
    memcpy(&buffer[3 + size + 4], &crc, 2);

    int result = write(fileStream, &buffer, (3 + size + 4 + 2));
    return 0;
}

int readData(int fileStream, char *bufferRetorno, int size){
    int tamanho = read(fileStream, bufferRetorno, size);
}
